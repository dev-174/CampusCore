...
import { IconGraduate, IconCheckSq, IconWarning, IconBuilding, IconSearch } from '../components/Icons';
...
export default function StudentsPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', department: '', batch: '' });
...
const [reportError, setReportError] = useState('');  
  ...
  async function handleAdd(e) {
    ...
    if (!form.name || !form.email || !form.department || !form.batch) {
      setFormError('Name, email, department, and batch are required.');
      return;
    }
    setSaving(true);
    setFormError('');
    try {
      ...
      setShowAdd(false);
      setForm({ name: '', email: '', department: '', batch: '' });
      fetchAll();
    } catch (e) {
      setFormError(e.response?.data?.error || e.response?.data?.detail || 'Failed to add student.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      const serverMsg = e.response?.data?.detail || e.response?.data?.error || 'Failed to delete student.';
      setError(serverMsg);
      setDeleteTarget(null);
    } finally {
      setDeleting(false);
  ...
  async function handleDownloadStudentReport(student) {
  setDownloadingStudentId(student.id);
  setReportError('');
  try {
    const safeName = (student.name || 'student').replace(/\s+/g, '_');
    await downloadPdf(`reports/student/${student.id}/pdf/`, `${safeName}_report_card.pdf`);
  } catch {
    setReportError('Failed to generate that student\'s report.');
  } finally {
    setDownloadingStudentId(null);
  }
}

async function handleDownloadClassReport() {
  if (!filterBatch) return;
  setDownloadingClass(true);
  setReportError('');
  try {
    const batchName = getBatchName(filterBatch).replace(/\s+/g, '_');
    await downloadPdf(`reports/class/pdf/?batch=${filterBatch}`, `class_report_${batchName}.pdf`);
  } catch {
    setReportError('Failed to generate the class report.');
  } finally {
  ...
  <div>
    {filterBatch ? (
      <button className="btn btn-primary btn-sm" onClick={handleDownloadClassReport} disabled={downloadingClass}>
        {downloadingClass ? <span className="spinner" /> : `⬇ Download Class Report — ${getBatchName(filterBatch)}`}
      </button>
    ) : (
      <p style={{ fontSize: '0.8rem', color: 'var(--text-3)' }}>Select a batch above to export a class report.</p>
    )}
  </div>
  {(search || filterDept || filterBatch || filterStatus) && (
    <button className="btn btn-ghost btn-sm" onClick={() => { setSearch(''); setFilterDept(''); setFilterBatch(''); setFilterStatus(''); }}>
      Clear Filters
    </button>
  )}
</div>
        ...
        <table className="students-table">
          <thead>
            <tr>
              ...
              <th className="col-enroll" style={{ whiteSpace: 'nowrap' }}>Enrollment No</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={s.id}>
                ...
                <td className="col-enroll" title={s.enrollment_number || 'Not Assigned'} style={{ whiteSpace: 'nowrap', color: s.enrollment_number ? 'var(--text-1)' : 'var(--text-3)', fontStyle: s.enrollment_number ? 'normal' : 'italic', fontWeight: s.enrollment_number ? 500 : 400 }}>
  ...
  <button
    ...
    onClick={() => handleDownloadStudentReport(s)}
    disabled={downloadingStudentId === s.id}
    title="Download report card"
  >
    {downloadingStudentId === s.id ? <span className="spinner" /> : '⬇'}
  </button>
</td>
              </tr>
            ))}
      ...
      <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title="Add Student">
        <form onSubmit={handleAdd}>
          ...
          <div className="form-group">
            ...
            <input className="form-input" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Aarav Sharma" />
          </div>
          <div className="form-group">
            <label className="form-label">Email</label>
            <input className="form-input" type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="aarav.sharma@student.edu" />
          </div>
